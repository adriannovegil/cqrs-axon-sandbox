...
/**
 * Record <code>HttpErrorInfo</code> which encapsulate all HTTP errors sent to
 * client.
 *
 * @implNote Since it is a record and not normal POJO, so it needs some
 * customizations to be serialized to JSON and this is done with method
